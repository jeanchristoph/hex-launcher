...
| File | Role |
|---|---|
| `install.bat` | **Double-click this**: opens the installation wizard (one window: detection, companion apps, shortcuts) |
| `LISEZMOI.txt` | Three-line quick start (FR/EN) |
| `app/` | The engine — nothing to edit there |
| `app/install.ps1` | The installation wizard (LoL-themed single window) |
| `app/config.json` | Riot paths + list of companion apps. Generated by `install.bat`, editable by hand if detection is not enough |
...
2. Double-click **`install.bat`** (never "Run as administrator": the tool refuses, on purpose). One window, three steps:
...
If detection got it wrong: edit `app\config.json` (see below) and run `install.bat` again. To force a fresh
