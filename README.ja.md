...
後から言語や補助アプリを追加・削除するには：**Hex Launcher** をダブルクリック（または `setup.bat` を再実行）してください。
...
1. LoL クライアントが起動していれば終了する — Riot Client のローカル API 経由でゲームセッションを正常に閉じる
   （Vanguard が接続中のプロセスを強制終了すると、短時間に数回繰り返した後に VAN 216 が出る）；強制終了は最後の手段。
   Riot Client が起動していなければ起動する。
...
3. 同じ API でゲームの起動を依頼し（**Play** ボタンと同じ動作）、クライアントが実際に起動したことを
   確認してから成功とみなす。
4. それ以外の場合は手動起動：Riot の全プロセスを終了し、`settings.locale` を書き換え（Riot が管理する
   `default_locale` は変更しません）、`--launch-product` で再起動します。あとは Riot Client で **プレイ** を
   押すだけなので、ランチャーが完全に失敗することはありません。`-NoLocalApi` でこの手動起動を強制できます。
   待機が長引く場合（2 分以上）、スプラッシュに **手動起動に切り替える** ボタンが表示されます。クリックすると
   今回の起動に限り手動起動に切り替わります。何も記憶されません。スプラッシュ右上の ✕ はランチャーを止めるだけです —
   何も閉じず、Riot とゲームはそのまま残るので、後でやり直せます。
   5 分以内にゲームを 4 回以上起動すると、終了方法にかかわらず Vanguard がエラー VAN 216 を表示してクライアントを
