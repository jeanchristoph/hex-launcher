...
- Au lancement du jeu, **une seule appli compagnon active** : le lanceur ferme les autres applis compagnon de `config.json` (toutes, si le raccourci n'en demande aucune) pour éviter les conflits d'overlay. Porofessor = client Overwolf.
- Propriété intellectuelle Riot (recherche du 2026-09-13) : aucune icône ni logo Riot dans le projet, aucune marque Riot dans le nom (« Hextech » est déposée) ; palette de couleurs tolérée ; mention Legal Jibber Jabber + non-affiliation obligatoires dans les README. Nom retenu : `hex-launcher`.
