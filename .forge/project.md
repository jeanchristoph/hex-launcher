...
└── app/                         # Tout le moteur — les scripts sont relatifs à $PSScriptRoot
    ...
    └── ico/                     # hex-launcher-setup.ico (engrenage du raccourci de configuration, commun) ; jeux d'icônes : un dossier par jeu (flat/ = défaut, généré par tools/make-flag-icons.ps1 ; classic/ = anciennes), hex-launcher[-xx].ico ; <jeu>/companion/ généré (gitignoré, hors release)
```
